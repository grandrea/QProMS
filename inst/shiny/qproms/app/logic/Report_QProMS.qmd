...
logic_dir <- file.path(app_dir, "app/logic")

old_wd <- getwd()
on.exit(setwd(old_wd), add = TRUE)
setwd(app_dir)

source(file.path(logic_dir, "R6Class_QProMS.R"), chdir = TRUE)

r6 <- QProMS$new()

rds_path <- file.path(logic_dir, "QProMS_session_internal.rds")
r6$loading_parameters(input_path = rds_path, self = r6)
